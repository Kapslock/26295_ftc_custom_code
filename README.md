## What's up Benson Robotics!!
This is a template repository, forked from the official FtcRobotController upstream repository. This one has custom structuring in the form of a dev/comp-ready branch setup, where changes can only be made to the dev branch, and when the code is ready for competition, you pull changes from the dev branch into the comp-ready branch. This usually involves creating a pull request, which must be approved by 1 other person. 

As a basic guide, add your team's coders to the respective 187/267 Github team under this organization, and put your most trusted and experienced ones in the 187/267 Captains team, which is hidden inside the respective normal team. Then, for each competition season or big off-season project, go to this template, click the "sync" button to make sure it's up to date with the official SDK. Click the button to "use this template" to make a new one, and make sure it's owned by the BensonRobotics organization, not yourself. Select the checkbox to copy the branches, so it maintains the branch naming and structuring. Name it like [team_number]-ftc-[comp_year] or ftc-[off_season_project_name] (or whatever you want), and remember, these template copies are NOT connected to the upstream, so for comps, wait until after kickoff when they publish the new SDK, THEN sync the template and copy it.

Now go into your new repository's settings, and under "collaborators", add your team to the repository with Write access, and add your Captains team with Maintain access. Optionally, for Gracious Professionalism points and ease of collaboration, add the other team to your repo with Read access. If you want any mentors to have access to your repository, add them to the Mentors team, and grant the Mentors team Triage access, which allows them to audit pull requests and other stuff, but they can't make changes to your code. This is defined in the FTC rulebook, stating that mentors and alumni can't directly contribute to your team's competition code. (This does mean they can have full access to any off-season code, which is in fact recommended.

Once you set up the access, you must then put a branch protection rule on your comp-ready branch, so that it cannot be deleted or directly changed. Go into your repo's settings, go to "rules", click "rulesets", make a new branch ruleset, name it something like protect-comp-ready, set it to active enforcement, allow organization admin to bypass it, and target your branch with a "comp-ready" pattern (or whatever your main branch is called). Set it to block deletions and force pushes, and require a pull request to be approved by 1 person. Save it, and you're done!

Now, just go to Android Studio, clone the repository using its Github URL, and you should be automatically checked into the dev branch. Do some coding, commit your changes at specific milestones, and push your local commits to the remote repository. Now other people can pull your commits to stay up to date, and you can too by doing the same.
